...
function getAllFromStore(db, storeName, userId) {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            const filteredData = request.result.filter(item => item.userId === userId);
            resolve(filteredData);
        };
        request.onerror = () => reject(new Error(`Erreur lors de la récupération des données dans ${storeName}`));
    });
}
...
async function generateStatistics(userId) {
    try {
        ...
        const activeBudgets = budgets.filter(b => Number(b.month) === currentMonth && Number(b.year) === currentYear && b.budget > 0);
    ...
    .filter(t => {
        ...
        return t.type === "debit" && activeBudgets.some(b => b.category === t.category);
    })
    .reduce((acc, t) => {
        // Check if `categories` is available and ensure proper lookup
        const categoryName = categories.find(c => c.id === t.category)?.name || "Inconnu";
...
if (recentTransactionsContainer) {
    recentTransactionsContainer.innerHTML = `
        ...
        <ul>
            ${activeBudgets.map(b => {
                const category = categories.find(c => c.id === b.category)?.name || "Inconnu";
        ...
        <ul>
            ${recentTransactions.map(t => {
                const category = categories.find(c => c.id === t.category)?.name || "Non défini";
