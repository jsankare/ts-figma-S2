...
async function displayUserProfile() {
  ...
  try {
    ...
    console.error("Erreur lors de l'affichage des informations de l'utilisateur:", error);
  }
  console.timeEnd("profile");
}

function updateUserProfileUI(user: any, db: IDBDatabase, userEmail: string) {
  const usernameElement = document.getElementById("profile--username") as HTMLHeadingElement;
  const profilePicture = document.getElementById("profile--picture") as HTMLImageElement;
  const addProfileButton = document.getElementById("button--addProfilePicture") as HTMLFormElement;
  const deletePicture = document.getElementById("delete-picture");
  const fileNameElement = document.getElementById("file-name") as HTMLParagraphElement;
  const fileTypeElement = document.getElementById("file-type") as HTMLParagraphElement;
  const fileSizeElement = document.getElementById("file-size") as HTMLParagraphElement;
  ...
  if (user.picture !== '') {
    ...
    if (deletePicture) {
      deletePicture.addEventListener("click", async () => {
        await handleDeletePicture(db, userEmail, profilePicture, addProfileButton, deletePicture);
      });
    }
  } else {
    ...
    addProfileButton.addEventListener("submit", async (event) => {
      ...
      await handleUploadPicture(db, userEmail, profilePicture, addProfileButton, fileNameElement, fileTypeElement, fileSizeElement);
    });
  }
}
...
async function handleUploadPicture(
  ...
  if (fileInput.files?.length) {
    ...
    try {
      ...
      toastAlert("error", "Erreur lors de la mise à jour de la photo de profil");
    }
  } else {
  ...
  return new Promise((resolve, reject) => {
    ...
    userRequest.onsuccess = () => {
      ...
      if (user) {
        ...
        updateRequest.onerror = () => reject("Erreur lors de la mise à jour de l'image de profil");
      } else {
        reject("Utilisateur introuvable");
      }
    };

    userRequest.onerror = () => reject("Erreur lors de la récupération de l'utilisateur");
  });
}
...
document.getElementById("take-photo-button")?.addEventListener("click", () => takePicture(video, canvas, stream));
...
async function startCamera() {
  ...
  try {
    ...
    // Afficher le bouton pour prendre une photo
    document.getElementById("take-photo-button")?.style.setProperty("display", "block");

    // Masquer le bouton de démarrage de la caméra
    document.getElementById("start-camera-button")?.style.setProperty("display", "none");
  } catch (error) {
...
function takePicture(video: HTMLVideoElement, canvas: HTMLCanvasElement, stream: MediaStream) {
  ...
  // Vérifier que la vidéo a bien une taille
  if (video.videoWidth === 0 || video.videoHeight === 0) {
    toastAlert("error", "Erreur : la vidéo n'a pas encore été chargée ou l'image est vide.");
    return;
  }
  ...
  // Convertir l'image du canvas en base64 pour l'envoi
  const pictureDataUrl = canvas.toDataURL("image/jpeg");
  const userEmail = localStorage.getItem("userMail");
  console.log("Email de l'utilisateur récupéré : ", userEmail);

  // Fermer le flux de la caméra
  const tracks = stream.getTracks();
  tracks.forEach(track => track.stop());

  if (userEmail) {
    // Mettre à jour l'image de profil avec la photo prise
