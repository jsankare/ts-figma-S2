  ...
  return `
    <article class="transaction-card" data-id="${transaction.id}">
      ...
      <div class="transaction-card-bottom">
        ...
        <div class="transaction-card-icons">
  ...
  for (const item of items) {
    if (isTransaction(item)) {
      try {
        ...
        const transactionHTML = createTransactionCard(item, category);
        const container = document.createElement("div");
        container.innerHTML = transactionHTML;
        const transactionElement = container.firstElementChild;
        ...
        if (transactionElement) {
          ...
          editBtn?.addEventListener("click", () => handleEdit(item, storeName));
          deleteBtn?.addEventListener("click", () =>
            handleDelete(item, dbName, storeName, keyPath, fields),
          );

          listing.appendChild(transactionElement);
        }
      } catch (error) {
        console.error("Error rendering transaction:", error);
      }
    } else if (isCategory(item) || isBudget(item)) {
      ...
      if (isCategory(item)) {
        ...
        try {
          ...
          let alertText = item.alert ? "Alerte activée" : "";
...
// Handle edit action
function handleEdit(item: Transaction | Category | Budget, storeName: string) {
  const modalContent = document.getElementById("modal");
  if (modalContent) {
...
async function handleDelete(
  item: Transaction | Category | Budget,
  dbName: string,
  storeName: string,
  keyPath: string,
  fields: string[],
  ...
  if (confirm("Are you sure you want to delete this item?")) {
    await deleteItem(dbName, storeName, item.id, fields);
    updateListing(dbName, storeName, keyPath, fields);
  }
}

// Update listing with items from database
...
function createEditButton(
  ...
  editButton.addEventListener("click", () => handleEdit(item, storeName));
  return editButton;
}
  ...
  inputElements.forEach((input) => {
    ...
    if (input instanceof HTMLInputElement && input.type === "file") {
      ...
      if (item[fieldName]) {
        ...
        if (fileNameField) {
          fileNameField.value = item[fieldName] as string;
        }
      }
    } else if (input instanceof HTMLInputElement && input.type === "checkbox") {
      (input as HTMLInputElement).checked = Boolean(item[fieldName]);
    } else {
