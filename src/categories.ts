...
async function handleFormSubmit(event: Event) {
  ...
  const originalCategoryName =
    (formData.get("originalCategoryName") as string) || categoryName;
      ...
      categories.forEach((category: { name: string; icon: string }) => {
        ...
        editButton.addEventListener("click", () => {
          ...
          (form.elements.namedItem("category") as HTMLInputElement).value =
            category.name;
          (
            form.querySelector('input[type="submit"]') as HTMLButtonElement
          ).value = "Mettre à jour";
          (
            form.elements.namedItem("originalCategoryName") as HTMLInputElement
          ).value = category.name;
        });
